...
from qfluentwidgets import (Dialog, FluentWindow, NavigationItemPosition, SplashScreen,
                            ...
                            ExpandLayout, isDarkTheme, SmoothScrollArea, FlowLayout, 
                            Flyout, PrimaryPushButton, PushButton)
...
class ExcelMatchWindow(FluentWindow):
    ...
    def _initHomeInterface(self):
        ...
        # 使用FlowLayout替代QVBoxLayout
        self.sheetSelectionLayout = FlowLayout(sheetSelectionWidget)
        self.sheetSelectionLayout.setContentsMargins(5, 5, 5, 5)
        self.sheetSelectionLayout.setHorizontalSpacing(10)
        self.sheetSelectionLayout.setVerticalSpacing(10)
        
        self.sheetSelectionScroll.setWidget(sheetSelectionWidget)
        self.sheetSelectionScroll.setMinimumHeight(120)  # 设置最小高度为三个下拉列表的高度
        ...
        leftLayout.addLayout(queryTitleLayout)
        ...
        leftLayout.addLayout(displayTitleLayout)
        ...
        self.matchFieldsLayout.setHorizontalSpacing(10)
        self.matchFieldsLayout.setVerticalSpacing(10)
    ...
    def _applyQueryConditions(self, df, query_fields):
        ...
        # 创建一个初始条件（全部为True）
        result_mask = pd.Series(True, index=df.index)
        current_logic = "且"  # 默认使用"且"逻辑
        
        # 应用每个查询条件
        for i, (column, operator, value, logic) in enumerate(active_query_fields):
            if i > 0:  # 第一个条件不需要考虑逻辑操作符
                current_logic = logic
            ...
            try:
                ...
                if operator == "包含":
                    ...
                    if is_datetime_column:
                        ...
                        try:
                            # 转换列为字符串以进行包含比较
                            datetime_col_str = df[column].astype(str)
                            current_mask = datetime_col_str.str.contains(value, case=False, na=False)
                        except:
                            # 如果失败，使用普通字符串比较
                        ...
                        try:
                            if pd.api.types.is_numeric_dtype(df[column]):
                                ...
                                current_mask = df[column].str.contains(value, case=False, na=False)
                    ...
                    if is_datetime_column:
                        ...
                        try:
                            ...
                            if operator == "大于":
                                current_mask = df[column] > numeric_value
                            elif operator == "小于":
                                current_mask = df[column] < numeric_value
                            elif operator == "大于等于":
                                current_mask = df[column] >= numeric_value
                ...
                # 处理任何其他异常
                raise ValueError(f"应用查询条件时出错: {str(e)}")
    ...
    def _addSheetSelection(self, sheet_name=None):
        ...
        # 将组件添加到布局，使用FlowLayout不需要addStretch
        self.sheetSelectionLayout.addWidget(sheetWidget)
    ...
    def _addQueryField(self):
        ...
        operatorCombo.addItems(["包含", "等于", "大于", "小于", "大于等于", "小于等于", "介于"])
    ...
    def displayResults(self, df):
        ...
        # 如果存在数据来源列，将其移到最左侧
        if '数据来源' in df.columns and list(df.columns).index('数据来源') > 0:
            # 获取所有列名
